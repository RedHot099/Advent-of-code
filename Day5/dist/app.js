...
const range = input[0].reverse()[0].trim().split("  ").map(e => Number(e)).reverse()[0];
//create payload array
let payload = [...Array(range)].map(_ => Array());
input[0].forEach(e => {
    for (let i = 0; i < range; i++) {
        if (e[1 + i * 4] != " ")
            payload[i].push(e[1 + i * 4]);
    }
});
//copy payload array
const payload1 = payload.map(arr => { return arr.slice(); });
